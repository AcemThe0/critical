...
public class Mhop extends Mod {
    private ModeSetting mode =
        new ModeSetting("Mode", "Mhop", "Mhop", "Strafe");
    private NumberSetting speed = new NumberSetting("Speed", 0, 5, 0.1, 0.1);
    private ModeSetting jumpMd =
        new ModeSetting("Jump", "Vanilla", "Vanilla", "Velocity");
    ...
    public Mhop() {
        ...
        addSettings(
            mode, speed, jumpMd, jump, jumpStr, new KeybindSetting("Key", 0)
        );
    }
    ...
    public void onTick() {
        ...
        case "Mhop":
            ...
            if (jump.isEnabled() &&
                (mc.player.forwardSpeed != 0 && mc.player.isOnGround() ||
                 mc.player.sidewaysSpeed != 0 && mc.player.isOnGround())) {
                if (jumpMd.getMode() == "Vanilla")
                    mc.player.jump();
                if (jumpMd.getMode() == "Velocity")
                    mc.player.addVelocity(0, jumpStr.getValueFloat(), 0);
            }
            super.onTick();
        ...
        case "Strafe":
            if (mc.player.forwardSpeed != 0 || mc.player.sidewaysSpeed != 0) {
                ...
                double vel = Math.abs(mc.player.getVelocity().getX()) +
                             Math.abs(mc.player.getVelocity().getZ());
