...
public class Killaura extends Mod {
    ...
    private ModeSetting speed =
        new ModeSetting("Speed", "Cooldwn", "Cooldwn", "Spam");
    private ModeSetting rotate =
        new ModeSetting("Rotate", "Smooth", "Snap", "Smooth", "None");
    private ModeSetting priority =
        new ModeSetting("Priority", "Angle", "Angle", "Health");
    ...
    public Killaura() {
        ...
        addSettings(
            range, fov, speed, rotate, priority, attack, lock, players,
            offensive, passive, new KeybindSetting("Key", 0)
        );
    }
            ...
            if (targets.size() >= 1) {
                ...
                if (!(rotate.getMode() == "None")) {
                    ...
                    if (rotate.getMode() == "Snap") {
                        ...
                        mc.player.setPitch(
                            (deltaPitch / 2.f) + mc.player.getPitch()
                        );
                    }
                }
    ...
    private void sortKA(List<Entity> targets) {
        for (Entity entity : mc.world.getEntities()) {
                    ...
                    float tYaw = Math.abs(
                        RotUtils.getYawToEnt(entity) - mc.player.getYaw()
                    );
                    float tPitch = Math.abs(
                        RotUtils.getPitchToEnt(entity, false) -
                        mc.player.getPitch()
                    );
                    if (entity instanceof LivingEntity && entity != mc.player &&
                        ...
                        tPitch <= tfov && tYaw <= tfov) {

                        if (players.isEnabled() && entity.isPlayer())
                            targets.add(entity);

                        // Will eventually split into finer categories (i.e.
                        // pigmen, endermen, villagers, etc.)
                        if (passive.isEnabled() &&
                            getEntityType(entity.getType()) == "Passive" &&
                            !entity.isPlayer())
                            targets.add(entity);
                        if (offensive.isEnabled() &&
                            getEntityType(entity.getType()) == "Offensive")
                            targets.add(entity);
                    }
                }
                targets.sort(
                    Comparator.comparingDouble(ent -> mc.player.distanceTo(ent))
                );
                switch (priority.getMode()) {
                case "Health":
                    targets.sort(Comparator.comparingDouble(
                        ent -> ((LivingEntity)ent).getHealth()
                    ));
                    break;
                case "Angle":
                    targets.sort(Comparator.comparingDouble(
                        ent -> RotUtils.angleDistance(ent, false)
                    ));
                    break;
                }
            }
