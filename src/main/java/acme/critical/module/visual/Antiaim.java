...
public class Antiaim extends Mod {
    ...
    private BooleanSetting actualAntiFuckingAim = new BooleanSetting("Position", false);
    ...
    public Antiaim() {
        ...
        addSettings(range, actualAntiFuckingAim, mode, spin, noForce, new KeybindSetting("Key", 0));
    }
    ...
    public void onTick() {
        if (actualAntiFuckingAim.isEnabled()) {
            if (mc.player.forwardSpeed != 0 || mc.player.sidewaysSpeed != 0) {lackPosition = true;}
            if(lackPosition) {
                x = mc.player.getX();
                z = mc.player.getZ();
                lackPosition = false;
    ...
    void randomTeleport(double x, double z, float range) {
        ...
        mc.player.networkHandler.sendPacket((Packet)new PlayerMoveC2SPacket.PositionAndOnGround(x+random(-1*range, range), mc.player.getY(), z+random(-1*range, range), true));
    }
}
