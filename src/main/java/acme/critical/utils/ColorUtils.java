...
public class ColorUtils {
    ...
    public static int Rainbow(int index, int speed, float saturation) {
        return Color.HSBtoRGB(
            ((System.currentTimeMillis() + index * 100) % (speed * 1000)) /
                4000f,
            saturation, 1
        );
    }

    public static int Rainbow() { return Rainbow(0, 4, 0.7f); }
    ...
    public static int getColorByCategory(String category) {
        ...
        return colors.get(category) != null ? (colors.get(category)) :
                new Color(0, 0, 0, 160).getRGB();
            }

            public static int arraylistColor(
                String mode, int rainbowIndex, String category
            ) {
